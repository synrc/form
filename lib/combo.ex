defmodule CRM.Forms.Combo do
  ...
  def dropDownList(filtered, field, module, feed) do
    formId = :form.atom([field, "form"])

    if filtered == [] do
      :nitro.wire("activeCombo = undefined; currentItem = undefined;")
      :nitro.hide(:form.atom([:comboContainer, field]))
      :nitro.wire("comboOpenFormById('#{formId}');")
    else
      :nitro.display(:form.atom([:comboContainer, field]), :block)
  ...
  def dropDown(obj, dom0, module, feed) do
    ...
    id = :nitro.jse(:erlang.iolist_to_binary(:form.atom([dom, :erlang.element(2, obj)])))
  ...
  def update_combo(id, feed, module), do: :nitro.update(:form.atom([:lookup, id]), NITRO.comboLookup(id: id, feed: feed, delegate: module))
